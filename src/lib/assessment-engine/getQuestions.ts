  ...
  switch (blockId) {
    case "A": return [
      ...
      { id: "A5", q: "Is the device subject to third-party conformity assessment under MDR/IVDR?", type: "yesno",
        skip: !hasEU,
        help: "Devices with third-party conformity assessment classify as high-risk under the EU AI Act (Article 6). Obligations apply from Aug 2, 2027 for Annex I devices. This applies under both MDR 2017/745 and IVDR 2017/746." },
    ...
    case "C": {
      if (isPMA) {
        return [
          ...
          { id: "C_PMA6", q: "Does this modification meet the EU AI Act definition of 'substantial modification'?", type: "yesnouncertain",
            ...
            help: "Under the EU AI Act Article 43(4), substantial modifications to high-risk AI systems may require a new conformity assessment. Note: pre-determined changes assessed at initial conformity assessment are exempt (MDCG 2025-6)." },
      ...
      return [
        ...
        { id: "C1", q: "Is this change solely to strengthen cybersecurity with no impact on device function or performance?", type: "yesno",
          skip: answers.B3 === Answer.Yes,
          help: "Pure cybersecurity patches with verified zero performance impact do not require a new 510(k). Per FDA Software Change Guidance (Oct 2017), Section V, Flowchart Question 1 — Cybersecurity Exemption. FDA requires the change to be 'solely to strengthen cybersecurity' with no other impact, supported by appropriate analysis/verification/validation. Note: Cybersecurity in Medical Devices: Quality Management System Considerations and Content of Premarket Submissions Guidance (Feb 2026), Section VII addresses §524B requirements for cyber devices." },
        { id: "C2", q: "Is this change solely to restore the device to its most recently cleared specification (i.e., a bug fix)?", type: "yesno",
          skip: answers.B3 === Answer.Yes || answers.C1 === Answer.Yes,
          help: "Bug fixes restoring the original cleared specifications do not require a new 510(k). Per FDA Software Change Guidance (Oct 2017), Section V, Flowchart Question 2 — Restore-to-Specification Exemption." },
        { id: "C3", q: "Does this change introduce a new or modified risk of harm with potential for patient injury?", type: "yesnouncertain",
          ...
          help: "Review your risk management file (ISO 14971 §7). A new or modified cause of a hazardous situation with significant unmitigated harm requires a new submission. Per FDA Software Change Guidance (Oct 2017), Section V, Flowchart Question 3 — risk assessment. The guidance asks whether the change introduces a new risk or modifies an existing risk that could result in significant harm. RegAssess decomposes Q3 into sub-questions for granularity: this question addresses new or modified causes of harm." },
        { id: "C4", q: "Does this change create an entirely new hazardous situation not present in the current risk analysis?", type: "yesnouncertain",
          ...
          help: "Distinct from the Cause of Harm question: this asks whether the change introduces an entirely new type of risk scenario. Per FDA Software Change Guidance (Oct 2017), Section V, Flowchart Question 3 — risk assessment (sub-question: new hazardous situation not in current risk analysis)." },
        { id: "C5", q: "Does this change create or necessitate a change to a risk control measure for a hazardous situation with significant harm potential?", type: "yesnouncertain",
          ...
          help: "If the change modifies, weakens, or removes an existing risk control, it may trigger regulatory significance. Per FDA Software Change Guidance (Oct 2017), Section V, Flowchart Question 3 — risk assessment (sub-question: risk control impact)." },
    ...
    case "D": return [
      ...
      { id: "D2", q: "Are system instructions, reasoning templates, or other controlled prompt/configuration elements being modified?", type: "yesno",
        help: "For generative AI medical devices, prompt configurations are part of the device design specification per FDA PCCP Final Guidance (Dec 2024, reissued Aug 2025), Section VI — Description of Modifications. These controlled elements should be version-controlled and validated." },
      ...
      { id: "D7", q: "Are all prompts, templates, and LLM configurations under version control as controlled documents in the QMS?", type: "yesno",
        help: "Version control of prompts and configurations is strongly recommended for PCCP eligibility for generative AI devices. FDA PCCP Final Guidance (Dec 2024, reissued Aug 2025) §VI treats prompts as part of device design specification. While not a separately enumerated FDA requirement, lack of version control undermines PCCP scope verification." },
      { id: "D8", q: "Does the change affect the explainability or interpretability of the model's outputs to clinicians?", type: "yesnouncertain",
        draftRef: true,
        help: "The FDA AI-DSF Guidance (Jan 2025 draft) emphasizes clinically relevant explanations appropriate for intended users. Changes that reduce explainability may affect safety." },
    ...
    case "F": return [
      ...
      { id: "F2", q: "Is the device authorized in Canada?", type: "yesno",
        skip: !hasCanada, help: "Health Canada has specific requirements for AI/ML device changes, including equity and bias assessment." },
    ...
    case "E": return [
      { id: "E1", q: "Does the training, validation, or test data adequately represent the device's intended patient population?", type: "yesnouncertain",
        draftRef: true,
        help: "FDA AI-DSF Guidance (Jan 2025 draft) recommends bias analysis across intended populations. Health Canada mandates equity assessment for AI/ML device changes." },
      { id: "E2", q: "Has subgroup performance been evaluated across relevant demographic groups (age, sex, ethnicity, comorbidities)?", type: "yesno",
        ...
        help: "Aggregate performance metrics can mask significant subgroup disparities. FDA expects subgroup analysis for all AI/ML submissions." },
