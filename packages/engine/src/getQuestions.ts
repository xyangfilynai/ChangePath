  ...
  const b: Block[] = [
    {
      ...
      icon: 'shield',
      description:
        'Record the authorized device, baseline, intended use, and change-control context before significance analysis.',
    },
    {
      id: 'B',
      label: 'Proposed change description',
      shortLabel: 'Change classification',
      icon: 'layers',
      description:
        'Describe the modification so downstream logic uses the correct change category and intended-use assessment.',
    },
    {
      id: 'C',
      label: 'Regulatory significance assessment',
      shortLabel: 'Regulatory significance',
      icon: 'alert',
      description:
        'Assess effects on safety, effectiveness, substantial equivalence, and reporting or submission obligations.',
    },
  ];
  if (ds.hasPCCP && answers.B3 !== Answer.Yes && answers.B3 !== Answer.Uncertain)
    b.push({
      id: 'P',
      label: 'PCCP scope verification',
      shortLabel: 'PCCP scope',
      icon: 'checkCircle',
      description:
        'Confirm the change stays within authorized PCCP boundaries, the validation protocol, and cumulative limits.',
    });
  if (ds.hasGenAI)
    b.push({
      id: 'D',
      label: 'GenAI supplemental assessment',
      shortLabel: 'GenAI supplemental',
      icon: 'cpu',
      description:
        'Review base model, prompts, RAG, guardrails, and other GenAI changes that may affect clinical performance or controls.',
    });
  b.push({
    id: 'E',
    label: 'Population and equity assessment',
    shortLabel: 'Bias and equity',
    icon: 'scale',
    description:
      'Assess population fit, subgroup performance, bias controls, and whether added expert review is warranted.',
  });
  b.push({
    id: 'review',
    ...
    icon: 'check',
    description:
      'Review the assessed pathway, open issues, regulatory source references, and reviewer notes before acting on the output.',
  });
  return b;
};
  ...
  switch (blockId) {
      ...
      return [
        {
          id: 'A_SEC1',
          sectionDivider: true,
          label: 'Pathway-critical fields',
          sublabel: 'These fields drive pathway logic and should match your authorization record.',
          icon: 'shield',
        },
        {
          id: 'A1',
          q: 'What is the current regulatory authorization pathway?',
          type: 'single',
        ...
        {
          id: 'A1b',
          q: 'What is the current U.S. authorization identifier (e.g., K#, DEN#, P#)?',
          type: 'text',
          pathwayCritical: true,
        ...
        {
          id: 'A1c',
          q: 'What is the authorized baseline being used for comparison (e.g., cleared/approved version, released build, or design baseline)?',
          type: 'text',
          pathwayCritical: true,
        ...
        {
          id: 'A1d',
          q: 'Paste or summarize the currently authorized indications for use (IFU) statement.',
          type: 'text',
          pathwayCritical: true,
        ...
        {
          id: 'A6',
          q: 'What type of AI/ML technology does the device use?',
          type: 'multi',
          options: [
        ...
        {
          id: 'A8',
          q: 'How many changes have been implemented since the last regulatory submission?',
          type: 'numeric',
          pathwayCritical: true,
    ...
    case 'B':
      return [
        {
          id: 'B1',
          q: 'What category of change is being made?',
          type: 'single',
        ...
        {
          id: 'B2',
          q: 'What is the specific type of change?',
          type: 'single',
          options: answers.B1
            ? changeTaxonomy[answerAsString(answers.B1)]?.types?.map((t: ChangeTypeDefinition) => t.name) || []
            : [],
          dynamic: true,
          disabled: !answers.B1,
          help: answers.B1
            ? 'Select the specific change type within the selected category.'
            : 'Select a change category first.',
          classificationGuidance: answers.B1
            ? changeTaxonomy[answerAsString(answers.B1)]?.classificationGuidance
            : null,
          boundaryNote: answers.B1 ? changeTaxonomy[answerAsString(answers.B1)]?.boundaryNote : null,
          selectedTypeData:
            answers.B1 && answers.B2
              ? changeTaxonomy[answerAsString(answers.B1)]?.types?.find(
                  (t: ChangeTypeDefinition) => t.name === answers.B2,
                )
              : null,
        },
        {
          id: 'B3',
          q: 'Does this change affect the intended use or indications for use?',
          type: 'yesnouncertain',
          critical: true,
          pathwayCritical: true,
          autoWarn: isCatIntendedUse
            ? "You selected 'Intended Use / Indications for Use' as the change category. This category frequently affects intended use — but you must explicitly confirm by comparing the proposed change against the authorized indications for use (IFU) statement word-by-word. Do not assume 'Yes' without that comparison."
            : null,
          forcedValue: null,
        ...
        {
          id: 'B4',
          q: 'Describe the change in detail.',
          type: 'text',
    ...
    case 'C': {
      if (isPMA) {
        return [
          {
            id: 'C_PMA1',
            q: 'Does this change affect the safety or effectiveness of the device?',
            type: 'yesnouncertain',
          ...
          {
            id: 'C_PMA2',
            q: 'Does this change affect the device labeling?',
            type: 'yesno',
            skip: answers.B3 === Answer.Yes,
            pathwayCritical: true,
          ...
          {
            id: 'C_PMA3',
            q: 'Does this change affect the manufacturing process or facility?',
            type: 'yesno',
            skip: answers.B3 === Answer.Yes,
            pathwayCritical: true,
          ...
          {
            id: 'C_PMA4',
            q: 'What type of PMA supplement is anticipated?',
            type: 'single',
            skip: answers.B3 !== Answer.Yes && answers.C_PMA1 !== Answer.Yes && answers.C_PMA1 !== Answer.Uncertain,
            options: [
              'Panel-Track Supplement (major change / typically substantial clinical evidence)',
              '180-Day Supplement (§814.39(a) — change affecting safety/effectiveness not requiring panel-track)',
              'Real-Time Supplement (minor change reviewed in real time; see FDA Real-Time PMA Supplements guidance)',
              'Special PMA Supplement — Changes Being Effected (§814.39(d) — certain safety-enhancing labeling or manufacturing changes)',
              '30-Day PMA Supplement (§814.39(e) — only when FDA has specifically advised this alternate submission)',
              '30-Day Notice (§814.39(f) — qualifying manufacturing procedure/method changes only)',
            ],
      ...
      return [
        {
          id: 'C0_DN1',
          q: 'Does the modified device still fit within the De Novo classification device type and special controls?',
          type: 'yesnouncertain',
        ...
        {
          id: 'C0_DN2',
          q: 'Can the modified device comply with all special controls specified in the De Novo classification order?',
          type: 'yesnouncertain',
        ...
        {
          id: 'C1',
          q: 'Is this change solely to strengthen cybersecurity with no impact on device function or performance?',
          type: 'yesnouncertain',
          skip: answers.B3 === Answer.Yes,
          pathwayCritical: true,
        ...
        {
          id: 'C2',
          q: 'Is this change solely to restore the device to its most recently cleared specification (i.e., a bug fix)?',
          type: 'yesnouncertain',
        ...
        {
          id: 'C3',
          q: 'Does this change introduce a new or modified risk of harm with potential for patient injury?',
          type: 'yesnouncertain',
          skip: answers.B3 === Answer.Yes || answers.C1 === Answer.Yes || answers.C2 === Answer.Yes,
          pathwayCritical: true,
          infoNote:
            answers.C3 === Answer.Yes || answers.C3 === Answer.Uncertain
              ? "Because this field was answered 'Yes' or 'Uncertain,' the remaining significance sub-fields (C4–C6) are not shown — the pathway determination is already triggered. However, for a complete regulatory record (Letter to File or submission), you should still evaluate and document all applicable risk dimensions (new hazardous situations, risk control impacts, and clinical performance effects) outside ChangePath."
              : null,
          mlguidance:
            "Could this change cause the model to miss diagnoses it previously caught? Run your existing test set through both versions — if the change creates a new clinically relevant error pattern, worsens a known failure mode, or leaves patient-harm risk unresolved, answer 'Yes' or 'Uncertain.'",
        ...
        {
          id: 'C4',
          q: 'Does this change create an entirely new hazardous situation not present in the current risk analysis?',
          type: 'yesnouncertain',
          skip:
            answers.B3 === Answer.Yes ||
            answers.C1 === Answer.Yes ||
            answers.C2 === Answer.Yes ||
            answers.C3 === Answer.Yes,
          pathwayCritical: true,
          mlguidance:
            'Review your risk management file for the current hazard list. Does this change create a failure mode NOT already documented? For architecture changes: new model families have different failure modes (e.g., transformers hallucinate differently than CNNs). For GenAI: check for novel hazard categories like fabricated clinical recommendations.',
        ...
        {
          id: 'C5',
          q: 'Does this change create or necessitate a change to a risk control measure for a hazardous situation with significant harm potential?',
          type: 'yesnouncertain',
          skip:
            answers.B3 === Answer.Yes ||
            answers.C1 === Answer.Yes ||
            answers.C2 === Answer.Yes ||
            answers.C3 === Answer.Yes ||
            answers.C4 === Answer.Yes,
          pathwayCritical: true,
          mlguidance:
            'Check your risk control matrix. If this change modifies guardrails, safety filters, confidence thresholds, human-in-the-loop overrides, or output constraints, those are risk controls. Also check: does the change weaken monitoring or anomaly detection that served as a risk mitigation?',
        ...
        {
          id: 'C6',
          q: 'Could this change significantly affect clinical functionality or performance specifications?',
          type: 'yesnouncertain',
          skip:
            answers.B3 === Answer.Yes ||
            answers.C1 === Answer.Yes ||
          ...
          pathwayCritical: true,
          mlguidance:
            "Compare performance metrics before and after on your holdout test set. If sensitivity, specificity, or any key metric changes beyond the predefined acceptance range, answer 'Yes.'",
        ...
        {
          id: 'C11',
          q: 'Is substantial equivalence to the predicate device still supportable after this change?',
          type: 'yesnouncertain',
    ...
    case 'P':
      return [
        ...
        {
          id: 'P2',
          q: 'Does the change fall within the predefined modification boundaries specified in the PCCP?',
          type: 'yesnouncertain',
        ...
        {
          id: 'P3',
          q: 'Are the PCCP-specified acceptance criteria and validation protocol available for this change?',
          type: 'yesno',
        ...
        {
          id: 'P5',
          q: 'Considering all previous PCCP-implemented changes, does this additional change still fall within the cumulative impact boundaries of the PCCP?',
          type: 'yesnouncertain',
          skip:
            answers.P1 !== Answer.Yes ||
    ...
    case 'D':
      return [
        {
          id: 'D1',
          q: 'Is the foundation or base model being changed (e.g., model version swap, provider change)?',
          type: 'yesno',
          mlguidance:
            "Evidence to gather: model version identifiers (before/after), upstream release notes, embedding dimension changes, tokenizer changes. Run your full evaluation suite on the new base model BEFORE fine-tuning to measure baseline drift. Document upstream provider's change log if available.",
        ...
        {
          id: 'D3',
          q: 'Is the retrieval-augmented generation (RAG) knowledge base or retrieval system changing?',
          type: 'yesno',
          help: 'Changes to the RAG knowledge base affect what information the model can access and reference for clinical decisions.',
        },
        {
          id: 'D4',
          q: 'Are safety guardrails, content filters, or output constraints being modified?',
          type: 'yesno',
          mlguidance:
            'Inventory all active guardrails: input validation, output filters, confidence thresholds, refusal rules, content classifiers. For each, document: is it being added, modified, or removed? What is the before/after behavior? Run adversarial test cases through the modified guardrail configuration.',
    ...
    case 'E':
      return [
        ...
        {
          id: 'E2',
          q: 'Has subgroup performance been evaluated across relevant demographic groups (age, sex, ethnicity, comorbidities)?',
          type: 'yesno',
          draftRef: true,
          mlguidance:
        ...
        {
          id: 'E3',
          q: 'Does this change introduce data from new demographic populations not represented in the original authorization?',
          type: 'yesnouncertain',
        ...
        {
          id: 'E5',
          q: 'Does the change affect any bias mitigation strategies that were part of the original device design?',
          type: 'yesnouncertain',
